...
def create_movie(
    ...
    lang: s.Language = s.Language.UK,
    # This flag determines whether the data is saved to a file and imported into a Google Sheet. False - for testing.
    save_to_json_and_google_sheet: bool = Query(default=True),
    db: Session = Depends(get_db),
):
    """Create a new movie"""
    ...
    try:
        ...
        if file:
            try:
                ...
                log(log.ERROR, "Error uploading poster [%s]: %s", form_data.key, e)
        ...
        if save_to_json_and_google_sheet:
            ...
            try:
                ...
                import_characters_to_google_spreadsheets(len(form_data.actors_keys), characters_data)
                ...
                for movie in movies:
                    movies_to_file.append(
                        s.MovieExportCreate(
                            ...
                            location_uk=next(
                                (t.location for t in movie.translations if t.language == s.Language.UK.value)
                            ),
                            location_en=next(
                                (t.location for t in movie.translations if t.language == s.Language.EN.value)
                            ),
                            # Genres
                            genres_list=[
                                {
                                    genre.id: next(
                                        (
                                            mg.percentage_match
                                            for mg in db.query(m.movie_genres).filter_by(
        ...
        raise HTTPException(status_code=400, detail=f"Error creating movie - {e}")
