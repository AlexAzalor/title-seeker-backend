...
if TYPE_CHECKING:
    from .title_category_translation import TitleCategoryTranslation
    from .title_criterion import TitleCriterion


class TitleCategory(db.Model, ModelMixin):
    ...
    translations: orm.Mapped[list["TitleCategoryTranslation"]] = orm.relationship()

    criteria: orm.Mapped[list["TitleCriterion"]] = orm.relationship(
        secondary=category_criteria, back_populates="categories"
    )

    def __repr__(self):
        return f"<TitleCategory [{self.id}]: {self.key}>"
